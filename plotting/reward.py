...
def _plot_single_average_reward(ax, budget, omv_avg, neural_cocoma_avg, random_avg, bench_avg, omv_std, neural_cocoma_std, random_std, bench_std, num_rounds, num_std_to_show, algorithm_colors):
    ...
    for i in range(len(omv_std)):
        if i == 0 or i % int(num_rounds / num_std_to_show) != 0 and i != len(omv_std) - 1:
            omv_std[i] = neural_cocoma_std[i] = random_std[i] = bench_std[i] = None
...
def plot_all_average_reward(processed_data, budgets, num_rounds, num_std_to_show, algorithm_colors, results_dir="."):
    ...
    for i, budget in enumerate(budgets):
        ...
        _plot_single_average_reward(
            ax, budget,
            budget_data['cocomama_avg_reward'],
            budget_data['neural_cocoma_avg_reward'],
            budget_data['random_avg_reward'], budget_data['bench_avg_reward'],
            budget_data['cocomama_std_reward'],
            budget_data['neural_cocoma_std_reward'],
